...
export class PlanetsService {
  apiAddress: string = 'https://swapi.dev/api/planets/';
  planetList$!: Observable<any>;
  dummyData = {
    count: 60,
  ...
  getName(url: string):string {
    let identity = url.substring(this.apiAddress.length, url.length - 1);
    let number = parseInt(identity);
    let counter = 0;
    let name = 'Ninguno'
    for (let index = 0; index < this.dummyData.results.length; index++) {
      if(counter==number){
        name = this.dummyData.results[index-1].name;
       }
      counter++;
  ...
  private getData():Observable<any> {
    return of(this.dummyData.results);
  }
  loadData():Planet[]{
